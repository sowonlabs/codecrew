...
/**
 * Register CodeCrew as MCP server for all available AI CLIs
 */
export async function registerCodeCrewMCP(): Promise<MCPRegistrationResult> {
...
/**
 * Register CodeCrew MCP server for Claude CLI
 */
async function registerClaudeMCP(): Promise<{ success: boolean; message: string }> {
  try {
    // Remove existing if any
    try {
      await execAsync('claude mcp remove codecrew -s user');
    } catch {
    ...
    return { 
      success: true, 
      message: 'Claude MCP registered successfully. Use: claude "Use codecrew_listAgents tool"' 
    };
  } catch (error: any) {
...
/**
 * Register CodeCrew MCP server for Gemini CLI
 */
async function registerGeminiMCP(): Promise<{ success: boolean; message: string }> {
  try {
    // Remove existing if any
    try {
      await execAsync('gemini mcp remove codecrew');
    } catch {
      // Ignore error if not exists
    }
    ...
    return { 
      success: true, 
      message: 'Gemini MCP registered successfully. Use: gemini -p "Use codecrew_listAgents" --allowed-mcp-server-names=codecrew --yolo' 
    };
  } catch (error: any) {
    return { success: false, message: error.message };
  }
...
/**
 * Register CodeCrew MCP server for Copilot CLI (manual JSON)
 */
async function registerCopilotMCP(): Promise<{ success: boolean; message: string }> {
  try {
    ...
    // Add CodeCrew MCP server
    config.mcpServers = config.mcpServers || {};
    config.mcpServers.codecrew = {
      command: 'codecrew',
      args: ['mcp']
    };
...
export async function checkMCPRegistration(): Promise<{
  ...
  try {
    ...
    if (fs.existsSync(claudeConfigPath)) {
      ...
      result.claude = content.includes('"codecrew"');
    }
  } catch {
    // Ignore error
  }
  ...
  try {
    ...
    result.gemini = stdout.includes('codecrew');
  } catch {
    // Ignore error
  }
  ...
  try {
    ...
    if (fs.existsSync(copilotConfigPath)) {
      const content = fs.readFileSync(copilotConfigPath, 'utf-8');
      result.copilot = content.includes('"codecrew"');
    }
  } catch {
    // Ignore error
  }
