...
export class ToolCallService {
  ...
  private codeCrewTool?: any; // Will be injected later to avoid circular dependency
  ...
  /**
   * Set CodeCrewTool reference (called after module initialization to avoid circular dependency)
   */
  setCodeCrewTool(codeCrewTool: any): void {
    this.codeCrewTool = codeCrewTool;
    // Register MCP tools after CodeCrewTool is available
  ...
  /**
   * Register MCP tools from CodeCrewTool
   */
  private registerMcpTools(): void {
    if (!this.codeCrewTool) {
      this.logger.warn('CodeCrewTool not available, skipping MCP tools registration');
      return;
    }
    ...
    this.register(
      ...
      },
      {
        execute: async (context: ToolExecutionContext): Promise<ToolExecutionResult> => {
          const startTime = Date.now();
          try {
            const result = await this.codeCrewTool.getTaskLogs({ taskId: context.input.taskId });
            return {
    ...
    this.register(
      ...
      },
      {
        execute: async (context: ToolExecutionContext): Promise<ToolExecutionResult> => {
          const startTime = Date.now();
          try {
            const result = await this.codeCrewTool.checkAIProviders();
            return {
