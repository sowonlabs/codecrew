...
export class HelpService {
  private getPackageInfo() {
    try {
      ...
      return {
        version: packageJson.version || '0.1.8',
        repository: packageJson.repository?.url || 'https://github.com/sowonlabs/codecrew'
      };
    } catch (error) {
      return {
        version: '0.1.8',
        repository: 'https://github.com/sowonlabs/codecrew'
      };
    }
  }
  ...
  CodeCrew CLI v${version} - AI-Powered Development Team at Your Fingertips
`));
    console.log(chalk.bold('========================================================='));
    console.log(chalk.yellow(`
Welcome to CodeCrew!

CodeCrew is a multi-AI agent collaboration platform designed to streamline your
...
`));
    ...
    console.log(chalk.white(`  $ codecrew <command> [options] "[prompt] [@agent1 @agent2 ...]"
`));
    ...
    this.showCommandHelp(
      'init',
      'Initializes a new CodeCrew project, creating a default `agents.yaml` configuration file.',
      ['codecrew init'],
    );
  ...
  ${chalk.gray('$ codecrew query "Review this code for bugs @claude and suggest performance improvements @gemini"')}
`));

    console.log(chalk.bold.white('\nOPTIONS:'));
    console.log(chalk.white(`  -c, --config <file>    Path to the agent configuration file (default: "agents.yaml")`));
